...
class StorageService {
  ...
  // Upload evidence image bytes to Firebase Storage.
  // Takes bytes instead of a File so this works on Flutter Web too -
  // dart:io File isn't available in a browser.
  Future<String> uploadEvidence(Uint8List imageBytes, String findingId) async {
    try {
      // Create a unique filename using timestamp
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final fileName = 'evidence/$findingId/$timestamp.jpg';
      ...
      // Upload the raw bytes (works on web, mobile, and desktop)
